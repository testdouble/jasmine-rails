...
function assertPageLoaded(status) {
  var globals = ['console_runner', 'jasmine'];
  for (x = 0; x < globals.length; x++) {
    var globalName = globals[x];
    var globalFound = page.evaluate(function() {
      return window[globalName];
    });
    if (!globalFound) {
      console.log("can't find global '" + globalName + "' object");
